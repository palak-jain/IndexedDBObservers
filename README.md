...
* This doc: https://goo.gl/Y2dobn
* Explainer: https://github.com/WICG/indexed-db-observers/blob/gh-pages/EXPLAINER.md
* Presentation: https://goo.gl/cYv4Xo
* Major Cls  
    - 2031113002 IDL constructs
    - 2062203004 Observer Lifetime Management
    - 2160163002 Propagating Changes to Observer : Browser
    - 2125213002 Propagating Changes to Observer : Renderer
    - 2163213006 Add Observer Test
...
2. Use Cases  
   i. Server Sync Case  
   ii. UI Update element  
   iii. Maintaining an in-memory data cache  
   iv. Custom refresh logic  
3. Interface  
4. Construction
  Observe call
...
Culling
FAQs
Why do both Observer object and database at the renderer hold observer ids?
A re-iteration of some messy orders
Why do we need to have an associated transaction?
Why not transaction.observe(..) or objStore.observe(..) ?
Why cannot we observe during version change?
...
https://github.com/WICG/indexed-db-observers/blob/gh-pages/EXPLAINER.md#ui-element
Maintaining an in-memory data cache 
IDB can be slow due to disk, so it's a good idea to have an in-memory cache
https://github.com/WICG/indexed-db-observers/blob/gh-pages/EXPLAINER.md#maintaining-an-in-memory-data-cache
Custom refresh logic
 If we just want to know when an object store has changed. This isn't the most efficient, but this might be the 'starting block' websites use to transition to observers, as at some point they would read the database using a transaction to update their UI.
	  https://github.com/WICG/indexed-db-observers/blob/gh-pages/EXPLAINER.md#custom-refresh-logic
...
observer.observe(db, tx)
Renderer
Each observer has an id unique to the renderer.
Observer is held in the renderer in a map of <id, object*>
We do bookkeeping of observer id in each Observer object as well as each database. (Needed for removal purposes)

Send an IPC message to browser with id and other filter options

Browser
Observer held on transaction as ‘pending’ . It does not start listening yet.
On completion of transaction, all associated ‘pending’ observers are ‘activated’ i.e. they start listening to database. These active observers are owned by the connection they were created on.  It is only at this point that the observer start listening to changes.


Removal

There are 3 cases of removal of observer:
Unobserve call  ( obs.unobserve(db) )
Connection closed by user (db.close() )
...
Unobserve : 
We require observer to be removed from an intersection of database and observer object. Remove all references of observer from renderer and then browser.
At the backend, if associated transaction has not yet finished, observer would have to be removed from pending observer list as well.



2. Connection close
We need all the observers observing on the database. That’s why (the only reason why) we hold their ids on database. 
Note that database does not have a reference to the observer object (IDBObserver) itself. So, we route through the dispatcher, get hold of the observer object via ids, and then remove ids from each observer object.




3. Force Close
This is different from above 2 cases since, removal initiates at the browser end. 
At browser, as connection is destroyed, observers are destroyed too.
At renderer, the entry point of observer destruction is database and process is similar to connection close.
...
Culling 
https://github.com/WICG/indexed-db-observers/blob/gh-pages/EXPLAINER.md#culling
...
More FAQs: https://github.com/WICG/indexed-db-observers/blob/gh-pages/EXPLAINER.md#faq
